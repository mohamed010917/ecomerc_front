...
function Month() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch("http://127.0.0.1:8000/api/monthes", {
          method: "GET",
  ...
  const chartData = {
    labels: data.map((item) => item.month || "غير معروف"),
    datasets: [
      {
        label: "order",
  ...
  const chartOptions = {
    ...
    plugins: {
      title: {
        ...
        text: "Top monthes order proudect", // ✅ نص العنوان
        font: {
          size: 18, // ✅ حجم الخط
  ...
  return (
    <div  style={{ width: "300px", height: "500px" }} className="p-2 m-2">
